...
nlr_model <- multinom(Survived ~ Fare + Fare2 + Sex + Pclass + Age+ Age*Pclass + Sex * Pclass, data = fdf)
...
```{r}
# Calculation and interpretation of marginal effects for the model (from the general-tospecific approach)

model <- mlogit(Survived~ 0| Fare + Family +Age, data = mldf)
z <- with(mldf, data.frame(Fare = tapply(Fare, index(model)$alt, mean), Family = tapply(Family, index(model)$alt, mean), Age = tapply(Age, index(model)$alt, mean)))
...
inter <- multinom(Survived~ Sex + Family + Pclass + Age + Sex*Age, data = fdf)

stargazer(model1, inter, model4, type="text")
...
The linktest is used to check the appropriate form of the model. The condition for linktest is to have yhat as significant and  
yhat^2 as insgnificant. 

Ho: The model has appropriate form

In the below model, we have yhat and yhat2 as significant. Therefore, we fail to reject the null hypothesis. 
...
mylogit <- multinom(Survived~ Sex + Pclass + Age + Family, data=fdf)
lt <- linktest(mylogit)
```
